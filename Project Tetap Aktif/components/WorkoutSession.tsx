          ...
          <div className="w-full aspect-video bg-black rounded-3xl overflow-hidden shadow-2xl mb-6 lg:mb-8 shrink-0 relative group">
            <video
              ...
              onError={(e) => {
                ...
                const fallback = document.getElementById('video-fallback-' + currentExercise.id);
            ...
            <button
              ...
              className="absolute bottom-3 right-3 bg-black/60 hover:bg-black/85 backdrop-blur-md p-2.5 rounded-full text-white/95 hover:text-white transition-all shadow-md active:scale-95 flex items-center justify-center cursor-pointer z-10"
              title={isMuted ? "Aktifkan Suara" : "Matikan Suara"}
            >
              {isMuted ? <VolumeX className="w-5 h-5" /> : <Volume2 className="w-5 h-5" />}
            </button>
            {/* Fallback if video fails */}
            <div id={`video-fallback-${currentExercise.id}`} className="absolute inset-0 bg-brand-brown/80 hidden flex-col items-center justify-center p-6 text-center">
               <p className="text-white font-bold mb-2 md:text-lg">Pratinjau Video Tidak Tersedia</p>
               <p className="text-white/70 text-sm mb-4">Silakan lihat panduan gerakan langsung di situs sumber.</p>
               <a 
